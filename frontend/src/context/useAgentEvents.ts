...
export function useAgentEvents({
  ...
  const dispatchStreamEvents = useCallback((eventType: string, data?: Record<string, unknown>): boolean => {
    switch (eventType) {
      case 'model_info':
  ...
  const dispatchToolAndSubAgentEvents = useCallback((eventType: string, data?: Record<string, unknown>): boolean => {
    switch (eventType) {
      case 'tool_call':
  ...
  const dispatchWorkflowEvents = useCallback((event: AgentEvent): boolean => {
    ...
    switch (eventType) {
      ...
      case 'plan_update': {
        ...
        const allCompleted = incomingTasks.length > 0 && incomingTasks.every((t) => t.status === 'completed' || t.status === 'cancelled');
        if (allCompleted) {
  ...
  }, [setApprovalEvent, setContextUsage, setCurrentConvStatus, setQuestionsPayload, setResources, setRightPanelOpen, setSearchBudget, setTasks, setTodoApprovalPayload, triggerFileTreeRefresh]);

  const dispatchSystemEvents = useCallback((eventType: string, data?: Record<string, unknown>, activeUuid?: string | null): boolean => {
    switch (eventType) {
      case 'turn_complete':
  ...
  }, [loadConversations, reloadConversationMessages, setApprovalEvent, setConvStatuses, setConversations, setCurrentConvStatus, setMcpServers, setMessages, setTodoApprovalPayload]);
